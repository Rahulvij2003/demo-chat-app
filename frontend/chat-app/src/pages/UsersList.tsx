...
const UserList = () => {
  ...
  const { users, fetchMessages, selectedUser } = useContext(ChatContext) as {
    users: User[];
  ...
  const { groups, createGroup, fetchGroups, setSelectedGroup } = useContext(GroupContext) as {
    groups: Group[];
    createGroup: (name: string, members: string[]) => Promise<{ group: Group }>;
    fetchGroups: () => void;
  ...
  <PlusIcon
    ...
    className="w-6 h-6 ml-2 cursor-pointer hover:text-blue-200 transition-transform duration-200 hover:scale-110"
    style={{ verticalAlign: "middle", minWidth: "24px", minHeight: "24px" }}
  />
)}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          {groups.map((g) => (
            <div
              key={g._id}
              onClick={() => {
                console.log("Group clicked:", g._id);
                fetchGroups();
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center rounded-full font-semibold text-white text-lg bg-gradient-to-tr from-green-400 to-teal-500">
                {getInitial(g.name)}
              </div>
